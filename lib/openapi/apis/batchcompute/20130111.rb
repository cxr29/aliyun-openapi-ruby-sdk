...
class OpenAPI::Batchcompute < OpenAPI::Service
	...
	def delete_image(resource_name, optional={})
	...
	#  name: _region, type: String
	#  name: _scheme, type: String, optional values: http|https
	def delete_snapshot(resource_name, optional={})
		args = self.class.new_params
		args[:method] = 'DELETE'
		args[:path]['ResourceName'] = resource_name
	...
	#  name: _region, type: String
	#  name: _scheme, type: String, optional values: http|https
	def get_image(resource_name, optional={})
		args = self.class.new_params
		args[:method] = 'GET'
		args[:path]['ResourceName'] = resource_name
		args[:pattern] = '/images/[ResourceName]'
		args[:query]['Action'] = 'GetImage'
		args[:region] = optional[:_region] if (optional.key? :_region)
		if optional.key? :_scheme
			raise ArgumentError, '_scheme must be http|https' unless 'http|https'.split('|').include? optional[:_scheme]
			args[:scheme] = optional[:_scheme]
	...
	#  name: _region, type: String
	#  name: _scheme, type: String, optional values: http|https
	def get_snapshot(resource_name, optional={})
		args = self.class.new_params
		args[:method] = 'GET'
		args[:path]['ResourceName'] = resource_name
		args[:pattern] = '/snapshots/[ResourceName]'
		args[:query]['Action'] = 'GetSnapshot'
		args[:region] = optional[:_region] if (optional.key? :_region)
		if optional.key? :_scheme
			raise ArgumentError, '_scheme must be http|https' unless 'http|https'.split('|').include? optional[:_scheme]
			args[:scheme] = optional[:_scheme]
