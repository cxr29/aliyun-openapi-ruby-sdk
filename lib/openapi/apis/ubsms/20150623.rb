...
class OpenAPI::Ubsms < OpenAPI::Service
	Product = 'Ubsms'
	Style = 'RPC'
	Version = '2015-06-23'
	...
	#  name: _region, type: String
	#  name: _scheme, type: String, optional values: http|https
	def set_user_business_status(service, status_key, status_value, uid, optional={})
		args = self.class.new_params
		args[:query]['Action'] = 'SetUserBusinessStatus'
		args[:query]['Service'] = service
		args[:query]['StatusKey'] = status_key
		args[:query]['StatusValue'] = status_value
		args[:query]['Uid'] = uid
		args[:region] = optional[:_region] if (optional.key? :_region)
		if optional.key? :_method
			raise ArgumentError, '_method must be GET|POST' unless 'GET|POST'.split('|').include? optional[:_method]
